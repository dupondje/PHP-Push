 ...
 /**
	...
	*/
	function check_webdav($path) {
		...
		// check schema
		if (preg_match('/1\s*,\s*2/', $resp['header']['dav'])) {
			return true;
		}
		...
		if ($response['status']['http-version'] == 'HTTP/1.1' ||
			...
			if (strcmp($response['status']['status-code'],'207') == 0 ) {
				...
				if (strcmp($response['header']['content-type'], 'text/xml; charset="utf-8"') == 0) {
					...
					// Free resources
					xml_parser_free($this->_parser);
					return $this->_ls[(int) $this->_parser];
				} else {
 ...
 /**
	...
	*/ 
	function _propfind_startElement($parser, $name, $attrs) {
		...
		if (!array_key_exists((int) $parser, $this->_xmltree)) {
	...
	function _process_respond() {
		...
		for($i=1; $i<count($lines); $i++) {
			...
			if (!$header_done ) {
				...
				if (!array_key_exists(strtolower($fieldname) ,$ret_struct['header'])) {
					$ret_struct['header'][strtolower($fieldname)] = trim($fieldvalue);
				} else {
				 $ret_struct['header'][strtolower($fieldname)] .= ',' . trim($fieldvalue);
				}
			}
